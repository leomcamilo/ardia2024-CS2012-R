...
A compreensão dos mecanismos que influenciam a formação de preços nos mercados financeiros é essencial para a análise da eficiência e liquidez destes mercados. Em particular, o spread bid-ask, definido como a diferença entre os preços de compra e venda de um ativo, desempenha um papel significativo na determinação dos custos de transação e na avaliação da liquidez de mercado. Este trabalho tem como objetivo principal investigar o impacto do spread bid-ask na formação de preços de ativos financeiros, com ênfase no mercado de ações brasileiro.

O estudo está estruturado em duas partes complementares. A primeira parte consiste em uma Revisão de Literatura, que busca explorar e sintetizar as principais teorias e metodologias relacionadas à estimativa do spread bid-ask. Nesta seção, serão abordados os fundamentos teóricos da microestrutura de mercado, incluindo a Hipótese de Eficiência de Mercado e os principais componentes do spread bid-ask. Além disso, serão revisados os modelos clássicos e contemporâneos de estimativa do spread, tanto em contextos de alta frequência quanto de baixa frequência, destacando suas aplicações e limitações em diferentes mercados financeiros. Espera-se que esta revisão forneça uma base sólida para compreender como os custos de transação influenciam a liquidez e a eficiência de mercado, bem como identificar as metodologias mais adequadas para estimar o spread bid-ask em diferentes contextos.

A segunda parte do trabalho é dedicada a um estudo empírico que aplica modelos de estimativa do spread bid-ask ao mercado de ações brasileiro. Nesta etapa, serão utilizados estimadores consagrados na literatura, como os modelos de Corwin e Schultz (2012), Abdi e Ranaldo (2017) e o estimador EDGE, para calcular o spread bid-ask a partir de dados de baixa frequência. O período de análise compreende os anos de 2017 a 2023, permitindo avaliar o comportamento do spread em diferentes cenários de mercado, incluindo períodos de alta volatilidade. O objetivo é verificar a aplicabilidade e a eficácia desses modelos no contexto brasileiro, identificando possíveis adaptações necessárias devido às particularidades do mercado local, como níveis diferenciados de liquidez, volatilidade e estrutura de negociação.

A segunda parte do trabalho é dedicada a um estudo empírico que aplica modelos de estimativa do spread bid-ask ao mercado de ações brasileiro. Nesta etapa, serão utilizados estimadores consagrados na literatura, como os modelos de Corwin e Schultz (2012), Abdi e Ranaldo (2017) e o estimador EDGE, para calcular o spread bid-ask a partir de dados de baixa frequência. O período de análise compreende os anos de 2017 a 2023, permitindo avaliar o comportamento do spread em diferentes cenários de mercado, incluindo períodos de alta volatilidade. O objetivo é verificar a aplicabilidade e a eficácia desses modelos no contexto brasileiro, identificando possíveis adaptações necessárias devido às particularidades do mercado local, como níveis diferenciados de liquidez, volatilidade e estrutura de negociação.

Ao confrontar os resultados obtidos com os apresentados na literatura internacional, pretende-se contribuir para uma melhor compreensão das dinâmicas do mercado acionário brasileiro e dos fatores que influenciam a formação dos spreads bid-ask. Ademais, a comparação entre os diferentes estimadores permitirá avaliar qual metodologia se adapta melhor às características do mercado nacional, fornecendo insights relevantes para investidores, gestores de risco e formuladores de políticas econômicas.

Este trabalho está organizado da seguinte forma: inicialmente, apresenta-se a Revisão de Literatura, abordando os principais conceitos teóricos e estudos empíricos relacionados ao spread bid-ask e à microestrutura de mercado. Em seguida, descreve-se a metodologia empregada no estudo empírico, detalhando os dados utilizados, os modelos de estimativa aplicados e os procedimentos de análise. Posteriormente, são discutidos os resultados obtidos, comparando-os com as evidências encontradas em outros mercados e destacando as implicações para o mercado brasileiro. Por fim, a conclusão sintetiza os principais achados do estudo, aponta as limitações encontradas e sugere direções para pesquisas futuras.

Dessa forma, este trabalho busca preencher lacunas na literatura sobre a estimativa do spread bid-ask no Brasil, oferecendo uma análise abrangente que combina teoria e prática. Ao explorar tanto os aspectos teóricos quanto a aplicação empírica dos modelos de estimativa, espera-se proporcionar uma contribuição significativa para o entendimento da formação de preços e da liquidez no mercado de ações brasileiro.
...
```{r img-grafbarravertmetod, echo = FALSE, fig.cap="Metodologias econométricas ou de cálculo do bid-ask spread dos artigos analisados no estudo"}
...
Em relação aos tipos de dados utilizados, a maioria dos estudos empregou dados de baixa frequência, como preços diários de abertura, fechamento, máximo e mínimo dos ativos negociados. A utilização de dados diários possibilitou uma análise detalhada do comportamento do bid-ask spread ao longo do tempo, oferecendo insights valiosos sobre sua variabilidade e impacto na formação de preços. Alguns estudos também utilizaram dados de alta frequência, especialmente em mercados mais líquidos, onde a análise de microestrutura do mercado é mais relevante. Essa combinação de dados de baixa e alta frequência forneceu uma base robusta para a investigação dos diferentes aspectos do bid-ask spread.

::: {#img-grafbarravertfreq}
```{r img-grafbarravertfreq, echo = FALSE, fig.cap="Dados das frequências dos dados analisados nos estudos"}
# Dados do gráfico
# Dados do gráfico
dados <- data.frame(
  Categoria = c("Baixa", "Alta", "Estudo teórico"),
  Valor = c(13, 4, 2)
)

# Criar o gráfico de barras ordenado do maior para o menor
ggplot(dados, aes(x = reorder(Categoria, -Valor), y = Valor)) +
  geom_bar(stat = "identity", fill = "darkblue", width = 0.35) +
  labs(
    title = "Frequência dos Tipos de Estudos",
    x = "Tipo de Estudo",
    y = "Quantidade"
  ) +
  theme_minimal()
...
Os mercados examinados pelos estudos incluíram tanto o mercado brasileiro quanto mercados internacionais. Um número igual de artigos focou-se no mercado de ações e debêntures no Brasil (SND), analisando dados da B3 (Bolsa de Valores do Brasil), e no mercado de ações dos Estados Unidos, especificamente a NYSE, somando 8 artigos cada. Além disso, alguns estudos seminais analisaram mercados europeus, oferecendo uma perspectiva comparativa sobre o bid-ask spread em diferentes contextos econômicos e regulatórios. Essa diversidade de mercados examinados permitiu uma compreensão mais ampla de como o bid-ask spread se comporta em diferentes ambientes de negociação, destacando variações regionais e estruturais.

::: {#img-grafbarravertmercest}
```{r img-grafbarravertmercest, echo = FALSE, fig.cap="Dados dos mercados analisados nos estudos"}
# Criar o data frame com os dados
mercados <- data.frame(
  Categoria = c("NYSE", "AMEX", "NASDAQ", "B3", "Turquia", "SND", "CRA na ANBIMA", "Sem bolsa específica"),
  Valor = c(8, 4, 2, 6, 1, 1, 1, 2)
)

# Criar o gráfico de barras
ggplot(mercados, aes(x = Categoria, y = Valor)) +
  geom_bar(stat = "identity", fill = "darkblue", width = 0.3) +
  labs(
    title = "Frequência dos Mercados Estudados",
    x = "Mercado",
    y = "Quantidade"
  ) +
  theme_minimal() +
...
Os aspectos do bid-ask spread analisados foram igualmente variados. Os estudos focaram-se em custos de transação, considerando os custos diretos associados ao bid-ask spread, como taxas de corretagem e impostos. A liquidez do mercado foi outro aspecto central, com ênfase na relação entre bid-ask spread e volume de negociação. Muitos estudos investigaram a assimetria de informação, analisando como a informação desigual entre compradores e vendedores afeta o bid-ask spread. A relação entre a volatilidade dos preços dos ativos e o bid-ask spread também foi amplamente examinada. Esses aspectos forneceram uma visão holística dos fatores que influenciam o bid-ask spread, elucidando sua importância para a eficiência e funcionamento dos mercados financeiros.

::: {#img-grafbarrahorizaspectos}
```{r img-grafbarrahorizaspectos, echo=FALSE, fig.cap="Aspectos Analisados nos Estudos"}
# Criar o data frame com os dados
aspectos <- data.frame(
  Categoria = c(
    "Riscos do Market Maker", "Volume de emissão", "Retorno das ações",
    "Custo de seleção adversa", "Tamanho da empresa", "Assimetria de informação",
    "Mercado não-eficiente", "Tamanho das negociações", "Custos de inventário",
    "Liquidez", "Pressupôs um mercado eficiente", "Volatilidade", "Custos de transação"
  ),
  Valor = c(1, 1, 3, 1, 1, 4, 1, 1, 1, 12, 2, 8, 14)
)

# Ordenar o data frame do maior para o menor valor
aspectos <- aspectos[order(aspectos$Valor, decreasing = TRUE), ]

# Criar o gráfico de barras horizontal
ggplot(aspectos, aes(x = Valor, y = reorder(Categoria, Valor))) +
  geom_bar(stat = "identity", fill = "darkblue", width = 0.25) +
  labs(
    title = "Aspectos analisados",
    x = "Quantidade",
    y = "Categoria"
  ) +
  theme_minimal()
...
Aspectos com relação ao bid-ask spread analisados nos estudos
:::
...
A estrutura do trabalho empírico é composta por várias seções, começando com a introdução ao tema e o contexto do mercado financeiro brasileiro. Em seguida, a seção de Relevância e Justificativa discute a importância de medir o bid-ask spread para investidores, reguladores e acadêmicos, destacando como este estudo preenche lacunas na literatura existente. A seção de Objetivos define as metas específicas do estudo, incluindo a comparação das metodologias selecionadas e a análise do comportamento do spread bid-ask em diferentes períodos.
...
O bid-ask spread é uma medida fundamental na microestrutura de mercado, representando a diferença entre os preços de compra (bid) e venda (ask) de um ativo. Este spread reflete os custos de transação e a liquidez do mercado, sendo influenciado por fatores como a volatilidade do ativo, a frequência de negociação e a assimetria de informações entre os participantes do mercado. A análise do bid-ask spread é crucial para entender a eficiência do mercado e os custos enfrentados pelos investidores.
