...
```{r calculo de liquidez, include=FALSE}
...
# Filtrar company_data_clean para manter apenas os códigos presentes em company_volumes$codigo
# Garante que company_volumes e a coluna 'codigo' existam
if (exists("company_volumes") && "codigo" %in% names(company_volumes)) {
  valid_codes <- company_volumes$codigo
  # Filtra a lista mantendo apenas os elementos cujos nomes estão em valid_codes
  company_data_clean <- company_data_clean[names(company_data_clean) %in% valid_codes]
} else {
  warning("O dataframe 'company_volumes' ou a coluna 'codigo' não foram encontrados. A filtragem não foi aplicada.")
}
